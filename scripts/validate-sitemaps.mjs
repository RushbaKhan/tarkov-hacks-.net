...
async function main() {
	...
	const redirects = await readFile(path.join(ROOT, 'src', 'redirects.ts'), 'utf8');
	...
	if (!redirects.includes("'/sitemap-index.xml': '/sitemap.xml'")) {
		fail('src/redirects.ts missing 301: /sitemap-index.xml → /sitemap.xml');
		bump();
	} else ok('Worker redirects 301 sitemap-index.xml → /sitemap.xml');
